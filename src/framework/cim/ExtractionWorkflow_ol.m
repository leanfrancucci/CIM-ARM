...
- (void) onDoorOpen: (DOOR) aDoor
{
	...
	if (myGenerateExtraction && generate ) {
		...
		myLastExtractionNumber = [myExtractionManager generateExtraction: myDoor user1: myUser1 user2: myUser2 bagNumber: myBagBarCode bagTrackingMode: myBagTrackingMode];
...
- (void) generateExtraction: (DOOR) aDoor
{
		...
		myDelayUser1 = NULL;
		myDelayUser2 = NULL;
		myLastExtractionNumber = [myExtractionManager generateExtraction: aDoor user1: myUser1 user2: myUser2 bagNumber: myBagBarCode bagTrackingMode: myBagTrackingMode];
}
...
//	doLog(0,"ExtractionWorkflow -> onDoorClose doorId %d\n", [aDoor getDoorId]);
	...
	if (myInnerDoorWorkflow) {
		if ([myInnerDoorWorkflow isDoorOpen]) {
			    //************************* logcoment
			//doLog(0,"ExtractionWorkflow -> onDoorClose: Debe cerrar primero la puerta interna.\n");
			[self buzzerStart];
		}
	}else{
		//[[Buzzer getInstance] buzzerStop];
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> waitUnlockDoorExit doorId = %d\n", [myDoor getDoorId]);
	[self stopTimer];
}
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> waitCloseDoorExit doorId = %d\n", [myDoor getDoorId]);
	[self stopTimer];
}
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> waitCloseDoorWarningEntry doorId = %d\n", [myDoor getDoorId]);
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> waitCloseDoorWarningExit doorId = %d\n", [myDoor getDoorId]);
	[self stopTimer];
	[[Buzzer getInstance] buzzerStop];
}
...
- (void) waitLockDoorEntry
{
	[self startTimer: LOCK_DOOR_WARNING_TIME * 1000];
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> waitLockDoorEntry doorId = %d\n", [myDoor getDoorId]);
}
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> waitLockDoorErrorEntry doorId = %d\n", [myDoor getDoorId]);
	[self startTimer: [self getFireAlarmTime] * 1000];
	[[Buzzer getInstance] buzzerStart];
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> doorLockAndOpenEntry doorId = %d\n", [myDoor getDoorId]);
	[self removeLoggedUsers];
...
{	
  ...
  int resId = RESID_NOT_DEFINE_VAL;
			    ...
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> doorViolationAction\n");


			    //************************* logcoment
//	LOG("******************* ACTIVA LA ALARMA ************************\n");
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> openDoorViolationEntry doorId = %d\n", [myDoor getDoorId]);
	[[Buzzer getInstance] buzzerStart];
	[self doorViolationAction];
...
//	LOG("ExtractionWorkflow -> openDoorViolationExit doorId = %d\n", [myDoor getDoorId]);
	[[Buzzer getInstance] buzzerStop];
}
...
{
			    //************************* logcoment
//	LOG("ExtractionWorkflow -> waitCloseDoorErrorEntry doorId = %d\n", [myDoor getDoorId]);
	[[Buzzer getInstance] buzzerStart];
	[self activateAlarm];
